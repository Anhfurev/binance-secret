...
export DB_SERVICE_ROLE_KEY="${DB_SERVICE_ROLE_KEY:-${SUPABASE_SERVICE_ROLE_KEY:-}}"
export BOT_SECRET="${BOT_SECRET:-${BINANCE_BOT_SECRET:-}}"
export BINANCE_BOT_SECRET="${BINANCE_BOT_SECRET:-${BOT_SECRET:-}}"
export BINANCE_SECRET="${BINANCE_SECRET:-${BINANCE_API_SECRET:-}}"
export BINANCE_API_SECRET="${BINANCE_API_SECRET:-${BINANCE_SECRET:-}}"
export FAST_BOUNCE_FUTURES_LANE="${FAST_BOUNCE_FUTURES_LANE:-0}"
# Stream hub on :8787 already has WS+klines — avoid 70+ sequential REST seeds per cron cold start.
export BINANCE_WS_MARKET_CACHE="${BINANCE_WS_MARKET_CACHE:-0}"
export MARKET_STREAM_PREFETCH_ENABLED="${MARKET_STREAM_PREFETCH_ENABLED:-1}"
export BINANCE_STREAM_TICK_GATEWAY_URL="${BINANCE_STREAM_TICK_GATEWAY_URL:-http://127.0.0.1:8787}"
