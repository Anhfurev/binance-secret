...
Deno.test("db stop hold defaults to three minutes", () => {
  Deno.env.delete("MIN_HOLD_BEFORE_DB_STOP_MS");
  assertEquals(readMinHoldBeforeDbStopMs(), 180_000);
});
...
Deno.test("db stop waits until min hold elapsed", () => {
  ...
  assertEquals(canFireDbStopLoss({ opened_at: openedAt } as any, now + 180_000), true);
});
