...
export function classifyWatchdogBlocker(reason: string): string | null {
...
export function aggregateWatchdogBlockers(
  ...
  for (const [reason, count] of blockerCounts.entries()) {
    ...
    if (!bucket) continue;
    totals[bucket as keyof typeof totals] += Number(count) || 0;
  }
  const rows = [
    {
      key: "warRoomHold",
